...
if(url.indexOf("home")!=-1 || url=="/index.php" || url=="/" || url==""){

    let shopButtons = $(".shop")

    shopButtons.click(openModalOrRedirect)

    function openModalOrRedirect(){
        $.ajax({
            url: "models/isLogIn.php",
            method: "GET",
            dataTyte: "json",
            data : {
                logovanje : 1
            ...
            success: function(x){
                if(x){
                    ...
                    $("#ispis").html("You must be logged in to access the shop page")
    ...
    function countOfStars(x){
       ...
       function gallery(){
            let trenutna = (this).src
            let trenutniNaslov = $(this).data("naslov")
            let naslov = $("#naslov")
            //console.log(trenutniNaslov)
            naslov.html(trenutniNaslov)
            let glavna = $("#glavna")
            glavna.attr("src", trenutna)
       }
}
//stranica za shop
        ...
        for(let element of x){
            ispis += `<div class="col-lg-3 col-sm-6 shop-info-grid text-center mt-4 art" > 
                        ...
                        <a class="link" href="index.php?page=single&id=${element.id_proizvod}">
                            ...
                            ${badge(prom[z])}
                                <img src="images/${element.slika}" class="img-fluid" alt="${element.naziv}">
                            </div>
                            <div class="item-info-product gore">
                                ...
                                <h6 class="mt-4">${isFreeShipping(cena[z].cena)}</h6>
                            </div>
                            </a>
                            </div>
...
if(url.indexOf("admin")==-1){
    ...
    var modalCart2 = document.getElementById("modalMail");
    var upis = document.getElementById("suc")
...
if(url.indexOf("login")!=-1 || url=="?page=login"){
    ...
    })//?page=admin&adminPage=addMorePool
}

if(url.indexOf("?page=admin&adminPage=addSurvey")!=-1 || url.indexOf("?page=admin&adminPage=addMorePool")!=-1 || url=="?page=admin&adminPage=addSurvey"){
    let dugme = $("#dodaj")

    dugme.click(function(){
        let dodatno = document.getElementById("dodatniOdgovori");

        var novoPolje = document.createElement("input");
        novoPolje.type = "text";
        novoPolje.placeholder = "Enter answer";
        novoPolje.classList.add("form-control");
        novoPolje.classList.add("mb-3");
        novoPolje.name = "odgovori[]";
        dodatno.appendChild(novoPolje);
...
$("#preusmeri").click(function(event){
    ...
    $.ajax({
        ...
        success:function(result){
            let modal = document.getElementById("modalKontakt")
            modal.classList.add("block")
            $("#ispis").html("You have successfully placed an order, one of the employees will contact you soon")
            let buttonCloseModal2 = document.getElementById("closeModal2")
            buttonCloseModal2.addEventListener("click",function(){
                modal.classList.remove("block");
                localStorage.removeItem("cart")
